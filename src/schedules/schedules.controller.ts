...
export class SchedulesController {
  ...
  @Get('schedules')
  async list(@Param('patientId') id: string, @CurrentUser() user: AuthUser) {
    await this.access.assertPatientReadAccess(user, id);
    return this.service.list(id);
  }

  @Roles(UserRole.PATIENT) @Post('schedules')
  create(@Param('patientId') id: string, @Body() body: CreateScheduleDto, @CurrentUser() user: AuthUser) {
    this.access.assertPatientWriteAccess(user, id);
    return this.service.create(id, body);
  }

  @Get('administrations')
  async history(@Param('patientId') id: string, @CurrentUser() user: AuthUser) {
    await this.access.assertPatientReadAccess(user, id);
    return this.service.history(id);
  }

  @Roles(UserRole.PATIENT) @Post('schedules/:scheduleId/occurrences')
  occurrence(@Param('patientId') id: string, @Param('scheduleId') scheduleId: string, @Body() body: OccurrenceDto, @CurrentUser() user: AuthUser) {
    this.access.assertPatientWriteAccess(user, id);
    return this.service.createOccurrence(id, scheduleId, new Date(body.scheduledFor));
  }
  ...
  async updateStatus(
    ...
    await this.access.assertPatientReadAccess(user, id);
    if (user.role === UserRole.CAREGIVER) await this.service.assertCaregiverHasActiveVisit(id, user.userId);
    return this.service.updateStatus(id, administrationId, body, user);
  }
}