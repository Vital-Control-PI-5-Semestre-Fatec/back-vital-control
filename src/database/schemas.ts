...
export class Medication {
  ...
  @Prop({ type: Object, required: true }) stock: {
...
export class MedicationSchedule {
  ...
  @Prop({ type: Object, required: true }) dose: {
    quantity: number;
    unit: string;
  };
...
export class MedicationAdministration {
  @Prop({ type: Types.ObjectId, ref: User.name, required: true, index: true })
  patientId: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: MedicationSchedule.name, required: true })
  scheduleId: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: Medication.name, required: true })
  medicationId: Types.ObjectId;
  @Prop({ required: true }) scheduledFor: Date;
  @Prop({
    enum: ['PENDING', 'TAKEN_ON_TIME', 'TAKEN_LATE', 'MISSED', 'SKIPPED'],
    default: 'PENDING',
  })
  status: string;
  ...
  @Prop() notes?: string;
  @Prop({ type: Object, required: true }) medicationSnapshot: Record<
    string,
    unknown
  >;
}
export const MedicationAdministrationSchema = SchemaFactory.createForClass(
...
export class HomeVisit {
  @Prop({ type: Types.ObjectId, ref: User.name, required: true, index: true })
  patientId: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: CareGroup.name })
  careGroupId?: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: User.name }) managerId?: Types.ObjectId;
  @Prop({ type: Types.ObjectId, ref: User.name, index: true })
  assignedCaregiverId?: Types.ObjectId;
  @Prop({ required: true }) reason: string;
  @Prop() patientNotes?: string;
  @Prop() caregiverNotes?: string;
  @Prop({ type: Object, required: true }) requestedWindow: {
    start: Date;
    end: Date;
  };
  @Prop({ type: Object }) scheduledWindow?: { start: Date; end: Date };
  @Prop({ type: Object, required: true }) addressSnapshot: Record<
    string,
    string
  >;
...
export const HomeVisitEventSchema =
  SchemaFactory.createForClass(HomeVisitEvent);
